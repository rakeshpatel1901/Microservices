...
public class ProductController {
    ...
    public ResponseEntity<ApiResponseDto<ProductDetailDTO>> getProductDetails(@PathVariable Long productId){
        ProductDetailDTO productDetailDTO = productService.getProductDetails(productId);

        ApiResponseDto<ProductDetailDTO> dto = new ApiResponseDto<>();
        dto.setData(productDetailDTO);
    ...
    public ResponseEntity<List<ProductCardResponseDTO>> getProductsByCategory(
            @RequestParam Long categoryId,
            @RequestParam(defaultValue = "7") int limit
    ) {
        List<ProductCardResponseDTO> products =
